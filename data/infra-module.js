...
function filtered(){
  ...
  out.sort(function(a,b){ var va=a[k], vb=b[k];
...
function render(){
  var rows=filtered(), km=0,monto=0,cons=0;
  rows.forEach(function(r){ if(r.km)km+=r.km; if(r.monto)monto+=r.monto; if(r.conserv)cons+=r.conserv; });
  document.getElementById('infra-t-n').textContent=rows.length;
  document.getElementById('infra-t-km').textContent=fmtKm(km);
  ...
  var html=rows.map(function(r){
    ...
    return '<tr'+sel+' data-id="'+r.id+'">'+
      ...
      '<td><span class="infra-badge '+tipoClass(r.tipo)+'" title="'+esc(r.tipo)+'">'+tipoLabel(r.tipo)+'</span></td>'+
      '<td>'+esc(r.comuna)+'</td>'+
      '<td title="'+esc(r.tramo)+'">'+esc(r.calle)+'</td>'+
      '<td class="infra-km">'+fmtKm(r.km)+'</td>'+
      '<td><span class="infra-est"><span class="infra-dot" style="background:'+estadoColor(r.estado)+'"></span>'+esc(r.estado)+'</span></td>'+
      '<td style="text-align:center">'+esc(r.anio)+'</td>'+
  ...
  document.getElementById('infra-tbody').innerHTML=html || '<tr><td colspan="8" style="text-align:center;padding:20px;color:#64748b">Sin resultados</td></tr>';
...
function makeResizable(panel, handle){
  ...
  handle.addEventListener('mousedown',function(e){
    resizing=true; var r=panel.getBoundingClientRect();
    panel.style.left=r.left+'px'; panel.style.right='auto'; panel.style.top=r.top+'px';
    sx=e.clientX; sy=e.clientY; sw=r.width; sh=r.height; e.preventDefault(); e.stopPropagation();
  });
  document.addEventListener('mousemove',function(e){ if(!resizing) return;
    var maxW=window.innerWidth-16, maxH=window.innerHeight-16;
    var w=Math.min(maxW, Math.max(320, sw+(e.clientX-sx)));
    var h=Math.min(maxH, Math.max(240, sh+(e.clientY-sy)));
...
function buildPanel(){
  ...
  p.innerHTML=
    ...
    '<div class="infra-totals">'+
      ...
      '<div class="infra-chip"><div class="v" id="infra-t-km">0</div><div class="l">Km</div></div>'+
    ...
    '<div class="infra-tablewrap"><table class="infra-tbl"><thead><tr>'+
      '<th data-k="kmz">KMZ</th><th data-k="tipo">Tipo</th><th data-k="comuna">Comuna</th><th data-k="calle">Calle</th><th data-k="km">Km</th><th data-k="estado">Estado</th><th data-k="anio">Año</th><th data-k="kmz" title="Geometría">Geo</th>'+
  ...
  var rz=document.createElement('div'); rz.className='infra-resize'; rz.id='infra-resize'; rz.title='Estirar'; rz.innerHTML='◢'; p.appendChild(rz);
  makeDraggable(p, document.getElementById('infra-head'));
  makeResizable(p, rz);
}
Infra.open=function(){ buildPanel(); buildFilterOptions(); render(); document.getElementById('infra-panel').classList.add('open'); var b=document.getElementById('btn-infra'); if(b) b.classList.add('infra-active'); };
...
function toggleMap(){
  if(Infra.map.shown){ if(Infra.map.group) Infra.map.group.clearLayers(); Infra.map.shown=false; document.getElementById('infra-tb-map').classList.remove('active'); }
...
Infra.onSelect=function(id){
  var editBtn=document.getElementById('infra-tb-edit'); if(editBtn) editBtn.disabled=!id; var edB=document.getElementById('infra-tb-editdata'); if(edB) edB.disabled=!id;
...
function ensureEditBar(){
  ...
  b.innerHTML='<b id="infra-eb-title">Editando</b> · Clic=agregar · arrastrar=mover · clic-der=borrar pto · <span>Km: <span class="km" id="infra-eb-km">0,00</span></span>'+
    '<button class="infra-eb-undo" id="infra-eb-undo">↩︎ Punto</button><button class="infra-eb-save" id="infra-eb-save">💾 Guardar</button><button class="infra-eb-undo" id="infra-eb-del" style="background:#7c3aed;color:#fff">🗑 Geom</button><button class="infra-eb-cancel" id="infra-eb-cancel">✕ Cancelar</button>';
  document.body.appendChild(b);
  document.getElementById('infra-eb-undo').onclick=undoVertex;
...
function removeVertex(idx){ Infra.edit.coords.splice(idx,1); var m=Infra.edit.markers.splice(idx,1)[0]; if(m) App.map.removeLayer(m); reindexMarkers(); refreshEditLine(); }
...
function startEdit(){
  ...
  ensureEditBar();
  Infra.edit.kmz=r.kmz; Infra.edit.coords=[]; Infra.edit.markers=[]; Infra.edit.line=null;
  var g=Infra.geom[String(r.kmz)];
  if(g&&g.coords){ g.coords.forEach(function(c){ addVertex(L.latLng(c[0],c[1])); }); }
...
function fetchDataJson(){
  ...
  try{
    fetch(DATA_URL,{cache:'no-store'}).then(function(r){ return r.ok?r.json():null; }).then(function(j){
      ...
      Infra.records=j.records; if(j.geom) Infra.geom=Object.assign({}, j.geom, Infra.geom);
...
function exportDataJson(){
  var data={records:Infra.records, geom:Infra.geom};
...
function ensureModal(){
  ...
  ov.innerHTML='<div class="infra-modal" id="infra-modal"><h3><span id="infra-modal-title">Editar</span><span class="x" id="infra-modal-x">✕</span></h3><div class="bd"></div><div class="ft"><button class="infra-mb-del" id="infra-mb-del">🗑 Borrar</button><div><button class="infra-mb-cancel" id="infra-mb-cancel">Cancelar</button> <button class="infra-mb-save" id="infra-mb-save">💾 Guardar</button></div></div></div>';
...
function openEditForm(rec){
  ensureModal();
  var isNew=!rec;
  if(isNew) rec={id:nextId(),kmz:'',seccion:'ACTUALES',tipo:TIPO_OPTS[1],comuna:'',calle:'',bip:'',km:'',tramo:'',estado:'Ejecutado y en Operación',anio:'',uf:'',monto:'',conserv:'',obs:''};
  Infra._editRec=rec; Infra._editNew=isNew;
  ...
  document.getElementById('infra-modal-title').textContent=isNew?'➕ Nuevo tramo':('✏️ Editar KMZ '+rec.kmz+' — '+(rec.calle||''));
...
function saveEditForm(){
  ...
  notify(Infra._editNew?('Tramo KMZ '+rec.kmz+' creado'):('KMZ '+rec.kmz+' actualizado'));
}
...
function init(){
  loadSeed(); loadOverrides(); loadRecordsOverride(); fetchDataJson();
  var tries=0;
