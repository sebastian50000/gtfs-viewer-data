...
function render(){
  ...
  document.getElementById('infra-tbody').innerHTML=html || '<tr><td colspan="9" style="text-align:center;padding:20px;color:#64748b">Sin resultados</td></tr>';
...
function startCellEdit(td){
  ...
  var ctrl;
  if(field==='estado'||field==='tipo'){
    var opts=(field==='estado')?estadoOpts():TIPO_OPTS;
...
function buildPanel(){
  ...
  p.innerHTML=
    ...
    '<div class="infra-totals">'+
      ...
      '<div class="infra-chip"><div class="v" id="infra-t-km-traz">0</div><div class="l">Km traz.</div></div>'+
    ...
    '<div class="infra-tablewrap"><table class="infra-tbl"><thead><tr>'+
      '<th data-k="kmz">KMZ</th><th data-k="tipo">Tipo</th><th data-k="comuna">Comuna</th><th data-k="calle">Calle</th><th data-k="km">Km hoja</th><th title="Km calculado del trazado (no se descarga aquí)">Km traz.</th><th data-k="estado">Estado</th><th data-k="anio">Año</th><th data-k="kmz" title="Geometría">Geo</th>'+
...
Infra.onSelect=function(id){
  ...
  var idx=id?Infra.records.findIndex(function(x){return x.id===id;}):-1;
  var upB=document.getElementById('infra-tb-up'); if(upB) upB.disabled=(idx<=0);
  var dnB=document.getElementById('infra-tb-down'); if(dnB) dnB.disabled=(idx<0||idx>=Infra.records.length-1);
...
function copyRecord(){
  ...
  var newKmz=prompt('Nuevo numero KMZ para la copia:', orig.kmz+1);
  if(newKmz===null||newKmz==='') return;
  newKmz=parseInt(newKmz,10); if(isNaN(newKmz)){ notify('Numero KMZ invalido'); return; }
  if(Infra.records.some(function(r){return r.kmz===newKmz;})){ notify('El KMZ '+newKmz+' ya existe'); return; }
  var idx=Infra.records.indexOf(orig);
  var copy=Object.assign({}, orig);
  copy.id=nextId(); copy.kmz=newKmz;
  Infra.records.splice(idx+1, 0, copy);
  persistRecords(); Infra.selectedId=copy.id; render();
  if(Infra.map && Infra.map.shown) drawLines();
  notify('Tramo KMZ '+newKmz+' copiado desde KMZ '+orig.kmz);
}
function moveUp(){
  var id=Infra.selectedId; if(!id) return;
  var idx=Infra.records.findIndex(function(x){return x.id===id;}); if(idx<=0) return;
  swap(Infra.records, idx, idx-1);
  persistRecords(); render();
  if(Infra.map && Infra.map.shown) drawLines();
}
function moveDown(){
  var id=Infra.selectedId; if(!id) return;
  var idx=Infra.records.findIndex(function(x){return x.id===id;}); if(idx<0||idx>=Infra.records.length-1) return;
  swap(Infra.records, idx, idx+1);
  persistRecords(); render();
  if(Infra.map && Infra.map.shown) drawLines();
}
...
function openEditForm(rec){
  ensureModal();
  var isNew=!rec;
  if(isNew) rec={id:nextId(),kmz:nextKmz(),seccion:'ACTUALES',tipo:TIPO_OPTS[1],comuna:'',calle:'',bip:'',km:'',tramo:'',estado:'Ejecutado y en Operación',anio:'',uf:'',monto:'',conserv:'',obs:''};
